...
# Gestión de Lenguajes de Programación en Manjaro Linux
...
1. **Instalación y Repositorio Oficial**:
   ```bash
   sudo pacman -Syu
   sudo pacman -S -y curl gpg
   sudo mkdir -p -m 755 /etc/pacman/keyrings
   curl -fsSL https://mise.jdx.dev/gpg-key.pub | sudo gpg --dearmor -o /etc/pacman/keyrings/mise-archive-keyring.gpg
   echo "deb [signed-by=/etc/pacman/keyrings/mise-archive-keyring.gpg arch=$(dpkg --print-architecture)] https://mise.jdx.dev/deb stable main" | sudo tee /etc/pacman/sources.list.d/mise.list > /dev/null
   sudo pacman -Syu
   sudo pacman -S -y mise
   ```
...
* **Instalación**: Configura la versión LTS 22 global:
  ...
  ```
* **Actualización segura de NPM**: Se aplica una limpieza de caché de npm e instalación previa de `promise-retry` para evadir errores clásicos del instalador de npm en Manjaro antes de subir a la versión más reciente (`npm install -g npm@latest`).
...
1. **Compiladores y Herramientas del Sistema**:
   ```bash
   sudo pacman -S -y build-essential cmake libssl-dev pkg-config curl
   ```
...
AutoFirma requiere interactuar con el almacén de claves NSS y la máquina virtual Java de Manjaro. Se instala a nivel de sistema APT:
```bash
sudo pacman -S -y default-jre default-jdk libnss3-tools
```
