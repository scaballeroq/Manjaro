...
# Configuración de Git en Manjaro Linux
...
1. **Dependencias y Clave GPG del repositorio oficial**:
   ...
   curl -fsSL https://cli.github.com/packages/githubcli-archive-keyring.gpg | sudo tee /etc/pacman/keyrings/githubcli-archive-keyring.gpg > /dev/null
   sudo chmod go+r /etc/pacman/keyrings/githubcli-archive-keyring.gpg
   ```

2. **Registro del Repositorio de GitHub**:
   ```bash
   echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/pacman/keyrings/githubcli-archive-keyring.gpg] https://cli.github.com/packages stable main" | sudo tee /etc/pacman/sources.list.d/github-cli.list > /dev/null
   ```
