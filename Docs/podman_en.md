...
# Container Management with Podman on Manjaro Linux
...
Installs Podman, its `compose` orchestration helper, and modern network stacks native to Manjaro Linux:
...
1. **Component Installation**:
   ...
   * **`passt` / `pasta`**: High-performance network stack for rootless containers, built-in by default starting with Podman 5 (standard in Manjaro Linux).
